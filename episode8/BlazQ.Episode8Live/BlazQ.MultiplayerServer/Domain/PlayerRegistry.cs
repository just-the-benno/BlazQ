...
namespace BlazQ.MultiplayerServer.Domain
{
    public class PlayerRegistry
    {
        ...
        internal List<ServerSidePlayerOverview> GetAllPlayersInLobby(Guid playerId)
        {
            if(_players.ContainsKey(playerId) == false)
            {
                return new List<ServerSidePlayerOverview>();
            }

            return _players[playerId].Lobby.Players.Select(x => new ServerSidePlayerOverview { Name = x.Name,  AvatarUrl = x.AvatarUrl, Id = x.Id }).ToList();
        }
    }
}
